...
export class getTaskListController implements IController {
    constructor(
        private getTaskListUseCase: IGetTaskListUseCase,
    ...
    async handle(request: IHttpRequest): Promise<IhttpResponse> {
        ...
        if(request.params && Object.keys(request.params).length > 0) {
            ...
            if (pathStringParams.includes('id')) {
                ...
                response = await this.getTaskListUseCase.execute(id);
            } else {
